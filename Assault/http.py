...
def fetch(url):
    """Make the requests and return the results"""
    started_at = time.monotonic()
    response = requests.get(url)
    request_time = time.monotonic() - started_at
    return{"status code": response.status_code, "request_time": request_time}


async def worker(name, queue, results):
    """A function tp make requests from a wueue and perform the work and then add results to the results list."""
...
async def distribute_work(url, requests, concurrency, results):
    """Divide work into batches and collect the final results"""
    queue = asyncio.Queue()
